...
async function updateOrderStatus(req, res) {
  ...
  if (!order_status || typeof order_status !== 'string' || order_status.trim() === "") {
    return res.status(400).json({ success: false, message: "Missing or invalid 'order_status' in request body." });
  }

  // Optional: Validate allowed status values
  const allowedStatuses = ['Pending', 'Processing', 'Shipped', 'Delivered', 'Cancelled'];
  if (!allowedStatuses.includes(order_status)) {
    return res.status(400).json({ success: false, message: `Invalid order status. Allowed: ${allowedStatuses.join(", ")}` });
  }
