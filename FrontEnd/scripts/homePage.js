  ...
  async function loadTopSales() {
    try {
      ...
      products.forEach(product => {
        ...
        // Determine stock display and Buy Now button state
        let stockDisplay = "";
        let buyNowButton = "";

        if (product.stock_quantity === 0) {
          stockDisplay = `<p class="text-danger fw-bold">Out of Stock</p>`;
          buyNowButton = `<button class="btn btn-secondary w-100" disabled>Buy Now</button>`;
        } else {
          stockDisplay = `<p class="text-success">In Stock: ${product.stock_quantity}</p>`;
          buyNowButton = `<a href="#" class="btn btn-danger w-100 buy-now-btn" data-id="${product.product_id}">Buy Now</a>`;
        }
      ...
      // Add event listeners only to enabled Buy Now buttons
